...
const Profile = () => {
  ...
  useEffect(() => {
    ...
    const fetchProfileData = async () => {
      try {
        const response = await axios.get("https://reqres.in/api/users/1"); // API endpoint'i
        setProfileData(response.data.data); // API yanıtı 'data' içinde bulunuyor
        setLoading(false);
      } catch (error) {
        console.error("Error fetching data", error);
        setError("Bir hata oluştu."); // Hata mesajı
        setLoading(false);
      }
    };
      ...
      {profileData ? (
        <div className="w-full max-w-5xl mb-4 grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="p-4 rounded-lg">
            ...
            <p className="text-lg">{t("profile.aboutMe2")}</p>
          </div>
        </div>
      ) : (
        <p>{t("Loading...")}</p>
